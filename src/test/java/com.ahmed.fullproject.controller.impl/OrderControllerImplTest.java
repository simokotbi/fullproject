...
import com.ahmed.fullproject.controller.impl.OrderControllerImpl;
...
public class OrderControllerImplTest {
    ...
    public void getById() throws Exception {
        ...
        Mockito.when(orderService.findById(ArgumentMatchers.anyInteger())).thenReturn(java.util.Optional.of(OrderBuilder.getEntity()));
    ...
    public void update() throws Exception {
        Mockito.when(orderMapper.asEntity(ArgumentMatchers.any())).thenReturn(OrderBuilder.getEntity());
        Mockito.when(orderService.update(ArgumentMatchers.any(), ArgumentMatchers.anyInteger())).thenReturn(OrderBuilder.getEntity());
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.put(ENDPOINT_URL + "/1")
                        ...
                        .content(CustomUtils.asJsonString(OrderBuilder.getDTO())))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(orderService, Mockito.times(1)).update(ArgumentMatchers.any(Order.class), ArgumentMatchers.anyInteger());
        Mockito.verifyNoMoreInteractions(orderService);
    }
