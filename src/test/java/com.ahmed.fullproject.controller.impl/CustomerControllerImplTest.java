...
import com.ahmed.fullproject.controller.impl.CustomerControllerImpl;
...
public class CustomerControllerImplTest {
    ...
    public void getById() throws Exception {
        ...
        Mockito.when(customerService.findById(ArgumentMatchers.anyInteger())).thenReturn(java.util.Optional.of(CustomerBuilder.getEntity()));
    ...
    public void update() throws Exception {
        Mockito.when(customerMapper.asEntity(ArgumentMatchers.any())).thenReturn(CustomerBuilder.getEntity());
        Mockito.when(customerService.update(ArgumentMatchers.any(), ArgumentMatchers.anyInteger())).thenReturn(CustomerBuilder.getEntity());
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.put(ENDPOINT_URL + "/1")
                        ...
                        .content(CustomUtils.asJsonString(CustomerBuilder.getDTO())))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(customerService, Mockito.times(1)).update(ArgumentMatchers.any(Customer.class), ArgumentMatchers.anyInteger());
        Mockito.verifyNoMoreInteractions(customerService);
    }

    @Test
    public void delete() throws Exception {
        Mockito.doNothing().when(customerService).deleteById(ArgumentMatchers.anyInteger());
        mockMvc.perform(
                MockMvcRequestBuilders.delete(ENDPOINT_URL + "/1"))
                .andExpect(MockMvcResultMatchers.status().isOk());
        Mockito.verify(customerService, Mockito.times(1)).deleteById(Mockito.anyInteger());
        Mockito.verifyNoMoreInteractions(customerService);
    }